...
Given /^my blitz contributions equal (\d+)$/ do |contributes|
  @user.blitz_contributes.should == contributes.to_i
end

Given /^my blitz rewards equal (\d+)$/ do |rewards|
  @user.blitz_rewards.should == rewards.to_i
end

When /^I make a payment of (\d+)$/ do |payment|
  @payment = @user.payments.create!(
    :group_id => 0,
    :amount => payment.to_i,
    :success => false
  )
  @payment.process_payment!.should be_true
end
...
Then /^I should have a new total blitz contribution of (\d+)$/ do |new_total|
